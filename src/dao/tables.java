        ...
        try {
            ...
            if (!tableExists(st, "userdetails")) {
                st.executeUpdate("CREATE TABLE userdetails(id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255) not null, "
                        ...
                        + "address VARCHAR(500) not null, province VARCHAR(100), country VARCHAR(100), uniqueregid VARCHAR(100) not null, "
                        + "imagename VARCHAR(100) );");
            }
            if (!tableExists(st, "userattendance")) {
                st.executeUpdate("CREATE TABLE userattendance (userid INT NOT NULL, date DATE NOT NULL, "
                        + "checkin DATETIME, checkout DATETIME, workduration VARCHAR(100));");
            }
